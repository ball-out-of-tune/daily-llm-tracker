...
def build_prompt(repo_name: str, commits: list[dict]) -> str:
    ...
    parts.append("""---
请从新手角度对以上每个 commit 做详细解读。输出必须严格按以下顺序和格式：

## 💡 今日关键词（放在最前面！）

先通读所有 commits，提炼出 3-5 个今日社区最关注的技术关键词/趋势。每个关键词包含：
- **关键词**：一句话通俗解释
- **为什么社区在关注**：这说明了什么趋势？比如"大家都在适配 AMD GPU，说明异构计算正在成为主流"
- **对新手意味着什么**：如果你正在学这个方向，意味着什么机会

## 🧪 重要知识点 & 动手实验

从今天的 commits 中挑出 4-5 个最具学习价值的技术概念，覆盖不同方向（分布式、量化、性能优化、模型适配等），为新手设计"动手实验"：

- **知识点 1：XXX**
  - **是什么**：一句话通俗解释
  - **为什么重要**：在真实场景中有什么用，不懂会踩什么坑
  - **动手试试**：具体可操作的实验步骤，最好是复制粘贴就能跑的代码或命令
  - **预期结果**：你会看到什么现象，说明你掌握了

- **知识点 2：XXX**
  （同样格式，选不同技术方向）

（以此类推，写满 4-5 个知识点，确保覆盖不同技术领域，让不同兴趣方向的新手都能找到自己关心的内容）

## 🔥 重要更新
（筛选 2-4 个最有影响的 commit，一两句话说明为什么值得关注）

## 📋 逐条解读

### 1. [commit 标题]
- **代码层面**：指出改的是哪个文件/函数，用通俗语言描述代码变更
- **新手概念课堂**：用生活比喻解释涉及的技术概念
- **对你有什么影响**：普通用户能感知的变化
